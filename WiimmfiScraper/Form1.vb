...
Public Class Form1
    ...
    Private Sub ParseHtmlAndSave(html As String)
                    ...
                    If rows IsNot Nothing Then
                        For Each row In rows
                            ...
                            If row.InnerText.Contains(targetFC) Then
                                ' Wir gehen alle Zellen der Reihe durch
                                Dim tds = row.SelectNodes(".//td")
                                If tds IsNot Nothing Then
                                    For Each td In tds
                                        Dim tooltip As String = td.GetAttributeValue("data-tooltip", "")
                                        If String.IsNullOrEmpty(tooltip) Then
                                            tooltip = td.GetAttributeValue("title", "")
                                        End If
                                        
                                        If tooltip.Contains(" @ ") Then
                                            Dim parts = tooltip.Split(New String() {" @ "}, StringSplitOptions.None)
                                            If parts.Length >= 2 Then
                                                driver = parts(0).Trim()
                                                vehicle = parts(1).Trim()
                                                Exit For ' Gefunden, weitere Zellen müssen nicht durchsucht werden
                                            End If
                                        End If
                                    Next
                                End If
        ...
        If foundRoom Then
            ...
            ' KORREKTUR: Zugriff auf rData statt auf die Variablen driver/vehicle, die hier nicht mehr existieren
            lblStatus.Text = $"Gefunden! ({rData("track")} - {rData("cc")} - {rData("driver")} / {rData("vehicle")})"
        Else
            File.WriteAllText(outputFile, "{}")
